...
cl winkill262.cpp /link psapi.lib
	...
	if(argc<4)
	{
		...
		printf("returns: 0 - ok\n"
			   ...
			   "         4 - deadlock ( system time limit exceeded )\n"
		           "  pass -java as the first option to disable exception handling\n"
			   "\n");
		...
		{ 
				case EXCEPTION_DEBUG_EVENT: 
					if( DebugEv.u.Exception.ExceptionRecord.ExceptionCode == EXCEPTION_BREAKPOINT )
					{
						...
						if (exception_handling && DebugEv.u.Exception.dwFirstChance == 0) {
							printf("Unhandled exception - runtime error.\n");
							bad=1;
						}
					}
					break;
